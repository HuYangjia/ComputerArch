...
注：编译代码时应该使用scons build/X86/gem5.debug -jX CPU_MODELS=AtomicSimpleCPU,TimingSimpleCPU,O3CPU,MinorCPU
...
2.  考虑一个实际情况，O3CPU 2.2GHz，issuewidth = 8。三种策略限制如下（只针对 l1dcache，这一步 l2cache 所有参数可以保持默认配置）：
...
1. 现在你的gem5应该是debug模式了。这个实验需要使用debug模式观察OpenMP加速过的demo程序。demo程序是给定的二进制可执行文件。

<br>

2. 考虑如下设定：4核2GHZ，O3CPU，l1d=l1i=1kB，l2=128kB，issue_width=8，l1d相联度=1，l1i相联度=4，l2相联度=4，替换策略使用默认LRU。使用debug模式模拟运行并输出你认为重要的cache行为到'cache_trace.txt'中。debug模式可以跟踪的选项见"--debug-help"。trace示例：
<br>
...
3. gem5使用了MOESI协议实现cache一致性。下面给出基础的MSI协议的行为表：
<br>
...
4. 这是MOESI的行为表，你的任务是将trace文件处理后从中寻找表中的各个行为，并根据你寻找的结果完成这个表的最后一列。提示：对一致性操作的观察主要围绕L1cache进行。

<br>

注意：每一个表项中都必须包含对你找到的有关trace语句的详细分析，如同层各cache之间的交互和上下层的交互，以及其中的详细行为如读/写地址，CPU编号，与总线的交互，发送的各种信号，状态转换等各种行为，同时以截图等形式记录你所找到的trace行的原始信息以方便检查。如果没有找到表中某行所对应的情况，请根据自己对于MOESI协议的理解尝试对该行的行为进行预测。
 <br>
...
1.  实验报告。内容包括NMRU的实现方法和修改配置的方法；关键代码；模拟结果展示；MOESI行为分析；实验过程中的问题（可选）。
2.  提交文件。所有经过修改的代码文件（可选，直接在报告中展示关键代码），模拟结果的配置和统计文件。trace文件太大可以不用提交，只提交你所使用的的trace行原始信息即可（截图或复制）
