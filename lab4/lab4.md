...
5.	对于代码中的三个函数，以循环展开为主要优化手段，可以加入其它优化手段，对函数进行重写，减少hazard。对于你修改后的循环优化函数，自己证明你所做的修改不影响函数的执行结果，即对于相同输入，改写后的函数仍能得到相同的结果。
6.	完成函数改写后请重新编译程序并使用gem5仿真执行，记录运行每一个函数所得的数据如CPI，执行时长，指令条数等。(注意指令条数请参考committedInsts统计项，而不要参考sim_insts统计项)。
...
9.	实验需要提交的内容包括:
    ...
    1、对于以上实验流程的展示
    2、对于以下问题的证明：
        1.	如何证明循环优化后的函数产生了正确的结果？
        2.	对于每一个函数，循环优化是否提升了性能？
        3.	循环展开减少了哪一种hazard？对于循环展开这一优化手段，你应该展开循环多少次？如果你没有展开足够多或展开太多会影响程序性能吗？
        4.	增加硬件对循环优化版本的函数和原函数有什么影响？添加更多硬件会减少哪种或哪些hazard？
        5.	选择你认为合适的指标比较四个版本函数的性能表现，为什么选择该指标？
        6.	你认为本次实验中你所进行的手动循环优化有意义吗？还是说编译器优化代码就已经足够了？
    你的结论应当有统计数据支撑。
...
2.	在完成第8步后，你应当对每一种函数都有了四个版本：(1)原函数;(2)手动循环优化函数;(3)GCC-优化后的原函数;(4)GCC-优化后的手动循环优化函数
...
## 实验附件
1.	本次实验会提供需要使用的交叉编译器(gcc-linaro-6.4.1-2017.11-x86_64_aarch64-linux-gnu.tar), 为了能够使用交叉编译器，你需要把提供的文件解压到ubuntu机器上，使用这个编译器来编译daxpy。
